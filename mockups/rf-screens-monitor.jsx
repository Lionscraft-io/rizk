// Monitor · Claims — AI/EO monitoring, live trigger conditions, claims history (merged screen)

const MonitorScreen = ({ goTo, phase }) => {
  const meta = PHASE_META[phase];
  const ai = AI_ASSESSMENT[phase];
  ...
  const watches = {
    ...
    event: [
      ...
      { title: "Hydro-drought — SLHDI", status: "EASING", note: "Heavy rain lifts catchment percentile — drought risk falls", warn: false, pct: 8 },
    ],
    payout: [
      ...
      { title: "Hydro-drought — SLHDI", status: "NOT MET", note: "Catchments refilled — opposite peril, no attachment", warn: false, pct: 6 },
    ],
  }[phase];
  ...
  return (
    <div className="page">
      <header className="page-head">
        <div>
          <div className="eyebrow">Monitor & Claims · EO + telemetry fusion · {meta.tag}</div>
          <h1 className="h-display" style={{ fontSize: 34 }}>See the event <em>as it happens.</em></h1>
        </div>
        <div className="meta">
          <div className="stat"><div className="label">Feeds Healthy</div><div className="val" style={{ color: "var(--accent)" }}>{SIGNALS.length} / {SIGNALS.length}</div></div>
          <div className="vdivider" />
          <div className="stat"><div className="label">Model Confidence</div><div className="val">{ai.confidence}%</div></div>
          <div className="vdivider" />
      ...
      {/* Map + AI assessment / watches */}
      <section className="grid mb-24" style={{ gridTemplateColumns: "1.35fr 1fr" }}>
        <div className="card">
          <div className="card-head">
            <div className="label">Situational Map — Sri Lanka · parametric box</div>
            <span className={cls("chip", meta.chipClass)}>{meta.chip}</span>
          </div>
          <LankaMap phase={phase} height={430} />
        </div>

        <div className="col gap-16">
          <div className="card">
            <div className="card-head">
              <div className="label">AI Situational Assessment</div>
              <span className="mono" style={{ fontSize: 10, color: "var(--ink-3)" }}>{meta.clock}</span>
            </div>
            <div className="card-pad">
              <div style={{ fontSize: 14.5, color: "var(--ink-0)", fontWeight: 500, lineHeight: 1.5, marginBottom: 10 }}>{ai.headline}</div>
              <div style={{ fontSize: 12.5, color: "var(--ink-2)", lineHeight: 1.6 }}>{ai.body}</div>
            </div>
          </div>
          ...
          <div className="card">
            <div className="card-head">
              ...
              <span className={cls("chip", watches.some(w => w.warn) ? "warn" : "live")}>{watches.filter(w => w.warn).length} active</span>
            </div>
            <div>
              {watches.map((w, i) => (
      ...
      <div className="card mb-24">
        <div className="card-head">
          ...
          <div className="mono" style={{ fontSize: 11, color: "var(--ink-3)" }}>Refreshing every 1.8s · cycle {phase === "calm" ? "6h" : "10min"}</div>
        </div>
      ...
      <div className="card card-pad" style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
        ...
        <div style={{ fontSize: 12.5, color: "var(--ink-2)", lineHeight: 1.6 }}>
          <strong style={{ color: "var(--ink-0)" }}>Who decides what.</strong> AI-assisted analysis supports event detection
          and measurement; operator telemetry is used under agreed data-sharing and cybersecurity controls. The parametric
          calculation is performed by the independent calculation agent under the bond terms, and payout authorisation
          rests with the Facility Governance Board.
        </div>
      </div>
    </div>
  );
...
Object.assign(window, { MonitorScreen, RFTriggerEventModal });
