...
const SL_SCREEN_KEY = "rizk-sl.screen";

function App() {
  const [screen, setScreen] = useS(() => {
    const valid = NAV_TABS.map(t => t.id).concat(["vault"]);
    try { const s = localStorage.getItem(SL_SCREEN_KEY); return valid.includes(s) ? s : "portfolio"; } catch (e) { return "portfolio"; }
  });
  useE(() => { try { localStorage.setItem(SL_SCREEN_KEY, screen); } catch (e) {} }, [screen]);
  ...
  const [payoutSim, setPayoutSim] = useS(false);
  ...
  const goTo = (s, ctx) => {
    ...
    if (s === "payouts" && ctx) setPayoutSim(true);
  ...
  return (
    <div className="app">
      ...
      <main className="main">
        ...
        {screen === "payouts" && <PayoutsScreen goTo={goTo} sim={payoutSim} setSim={setPayoutSim} />}
        {screen === "resilience" && <ResilienceScreen goTo={goTo} sim={payoutSim} />}
...
const TopNav = ({ screen, setScreen, side, setSide, showCedentToggle, theme, toggleTheme }) => (
  <nav className="nav">
    <div className="nav-brand" onClick={() => setScreen("portfolio")} style={{ cursor: "pointer" }}>
      ...
      <span className="mono" style={{ fontSize: 10, color: "var(--ink-3)", marginLeft: 8 }}>SRI LANKA</span>
    </div>
